                    ...
                    if (env.BUILD_ALL != "true" && changedServicesList.size() > 0) {
                        ...
                        for (service in changedServicesList) {
                            ...
                            if (jarFile && jarFile != '') {
                                ...
                                sh "ls -l ${service}.jar" // Debug: xác nhận file đã copy ra root
                            } else {
                                error "Could not find JAR file for ${service}. Build may have failed."
                            }
                        }
                        // Debug: liệt kê tất cả file JAR ở workspace root
                        sh "ls -l *.jar || true"
                    }
                }
            }
            ...
            steps {
                script {
                    ...
                    for (service in changedServicesList) {
                        ...
                        sh "ls -l ${service}.jar" // Debug: xác nhận file JAR tồn tại trước khi build
